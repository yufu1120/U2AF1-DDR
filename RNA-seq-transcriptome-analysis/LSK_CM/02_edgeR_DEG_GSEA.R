...

{
  ...
  CHW.IR.cpm<-cpm.all$DEG[,cm_count_columns]
  CHW.IR.cpm.df<-cpm.all$df[,cm_count_columns]
    ...
    # run GSEA
    {
      ...
      # C5
      {
        ...
        # plotting
        {
          ...
          gene_list<-openxlsx::read.xlsx("../gene_list_em_plot_2.xlsx")
          ...
          for (i in 1:length(em.sig.plot)){
            ...
            # FDR<-em@result$qvalues[em@result$ID==geneSetID]
            # FDR<-format(FDR, digits=4)
            p1<-gseaplot2(em, geneSetID=geneSetID, title=geneSetID, subplots = 1)+
              ...
              # annotate("text", x=0.95, y=0.85, label= paste("adj-p-value=",pvalue), hjust = 1)+
              # annotate("text", x=0.95, y=0.8, label= paste("FDR=",FDR), hjust = 1)
            p2<-gseaplot2(em, geneSetID=geneSetID, title=geneSetID, subplots = 2:3)+
              theme(plot.title = element_text(size=0.5))
