...
#SBATCH --output=./LSK_WT.log
#SBATCH --error=./LSK_WT.err

params_dir_ref="$(pwd)"
current_project=.
current_project_output=${current_project}/1
gtf=./refFlat_gene_mm10_200213.gtf

singularity exec --bind ${params_dir_ref} /opt/rMATS/4.1.2.simg python /rmats/rmats.py \
 --b1 ${current_project}/input/lnk/1_WT_b1_NIR.txt \
 --b2 ${current_project}/input/lnk/1_WT_b2_IR.txt \
