...
CONFIG <- list(
  # ----- paths -----
  rmats_root = ".",
  output_root = "rmats_integrated_output",#change the folder here
  gtf_file     = "./refFlat_gene_mm10_200213.gtf",
  genome_fasta = "./mm10.fa",   # REQUIRED for sequence extraction

  # ----- cohorts / comparison labels -----
  cohorts = c(
    "HDAC8_MIG" = "HDAC8_MIG",
    "HDAC8_OE" = "HDAC8_OE",
    "HDAC8_OE_vs_MIG" = "HDAC8_OE_vs_MIG"
  ),
  group1_label = "NIR",
  group2_label = "IR",
  ...
  plot_filters = list(
    ...
    gene_symbols = c("Brad1", "Brca1", "Atm", "Mdc1", "Uimc1", "Rad50", "Rad9a", "Rad9b", "Wrn", "Hells", "Chaf1a"),
    AS_type = NULL,
    cohort = NULL,
    filter_counts = TRUE,
    filter_PSI = TRUE,
...

{
  ...
  get_rmats_file <- function(root, cohort_dir, as_type, use_jcec = TRUE) {
    file.path(root, cohort_dir, "final_output", get_rmats_filename(as_type, use_jcec))
  }
  ...
  read_one_rmats <- function(file, cohort_name, as_type) {
    ...
    df %>%
      mutate(
        geneSymbol = dplyr::case_when(
          !is.na(geneSymbol) & geneSymbol != "" ~ geneSymbol,
          "GeneID" %in% colnames(df) & !is.na(GeneID) & GeneID != "" ~ GeneID,
          TRUE ~ geneSymbol
        ),
        cohort = cohort_name,
        AS_type = as_type,
        ...
        IncLevelDifference = safe_numeric(IncLevelDifference)
      )
  }
  ...
  build_analysis_table_one_cohort <- function(cohort_name, cohort_dir, config) {
    dfs <- lapply(config$all_as_types, function(as_type) {
      ...
      read_one_rmats(file, cohort_name = cohort_name, as_type = as_type)
    })
    ...
    bind_rows(dfs) %>%
      mutate(
        ...
        filter_PSI = abs(IncLevelDifference) >= config$cutoff_delta_psi,
  ...
  build_analysis_tables_all <- function(config) {
    ...
    all_tables <- imap(config$cohorts, function(cohort_dir, cohort_name) {
      df <- build_analysis_table_one_cohort(cohort_name, cohort_dir, config)
      prefix <- file.path(out_dir, sanitize_filename(paste0(cohort_name, ".analysis_ready")))
      write_table_files(df, prefix)
      df
    })
    bind_rows(all_tables)
  }
  ...
  classify_events <- function(df, config) {
    df %>%
      mutate(
        ...
        is_background_for_motif = filter_counts & (FDR > config$cutoff_fdr) & (abs(IncLevelDifference) < config$cutoff_delta_psi),
        event_class = case_when(
          is_significant_for_motif & IncLevelDifference > 0 ~ "more_skipped_in_group2",
          is_significant_for_motif & IncLevelDifference < 0 ~ "more_included_in_group2",
    ...
    load_longest_transcript_exons <- function(gtf_file) {
      ...
      exon_df <- gtf_df %>%
        ...
        dplyr::mutate(
          ...
          transcript_name = dplyr::case_when(
            !is.na(transcript_name) & transcript_name != "" ~ transcript_name,
            !is.na(transcript_id) & transcript_id != "" ~ transcript_id,
            TRUE ~ NA_character_
          )
        ) %>%
  ...
  filter_events_general <- function(df, gene_symbols = NULL, AS_type = NULL, cohort = NULL,
    ...
    if (!is.null(min_abs_dpsi)) out <- out %>% filter(abs(.data$IncLevelDifference) >= min_abs_dpsi)
  ...
  build_sashimi_manifest <- function(df, config) {
    ...
    filt <- filt %>% mutate(
      plot_subdir = file.path(as.character(AS_type), plot_id),
      expected_pdf_dir = file.path("output", as.character(AS_type), plot_id),
      ...
      ppt_subtitle = paste0(
        ...
        " | FDR=", fmt_num(FDR, 2),
        " | dPSI=", signif(IncLevelDifference, 3)
      )
    )
