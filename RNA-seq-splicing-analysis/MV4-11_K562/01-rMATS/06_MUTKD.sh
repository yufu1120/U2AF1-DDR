...
#SBATCH --output=./MUTKD.log
#SBATCH --error=./MUTKD.err

params_dir_ref="$(pwd)"
current_project=./6_MUTKD
current_project_output=./6_MUTKD/output
gtf=./genome.gtf

singularity exec --bind ${params_dir_ref} /opt/rMATS/4.1.2.simg python /rmats/rmats.py \
 --b1 ./lnk/MUTKD_b1_NIR.txt \
 --b2 ./lnk/MUTKD_b2_IR.txt \
